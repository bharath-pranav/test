...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'SDR Training' }).fill('test recording 1');
  await expect(page.getByRole('heading')).toContainText('Save Recording');
  await page.getByRole('button', { name: 'Save Recording' }).click();
  await page.getByTestId('button').nth(5).click();
  await expect(page.getByRole('main')).toContainText('test recording 1');
});