...
test('test', async ({ page }) => {
  ...
  await page.getByTestId('ask-nova-ai-button').click();
  await page.getByTestId('nova-input').click();
  await page.getByTestId('nova-input').fill('hi');
  await page.getByRole('button', { name: 'Send message' }).click();
  const dialogText = await page.getByRole('dialog').innerText();
// Checks if dialogText is not empty
