...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Search issues, authors, repos' }).fill('xyz123');
  await expect(page.getByRole('heading')).toContainText('No issues found');
  await expect(page.getByRole('paragraph')).toContainText('Try adjusting your filters or search criteria');
});