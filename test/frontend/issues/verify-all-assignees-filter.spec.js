...
test('test', async ({ page }) => {
  ...
  await page.getByLabel('All Assignees').getByText('agni-').click();
  await page.getByRole('button', { name: 'All Repositories' }).click();
  await page.getByText('surveysparrow-evals').click();
  await expect(page.locator('main')).toContainText('agni-1723');
  await expect(page.locator('main')).toContainText('surveysparrow-evals');
  await expect(page.locator('main')).toContainText('agni-1723');
});