...
test('test', async ({ page }) => {
  ...
  await expect(page.locator('main')).toContainText('Past 7 days');
  await expect(page.getByRole('main')).toContainText('In Progress - PR Created');
  await expect(page.getByRole('main')).toContainText('In Progress - PR Created');
  await expect(page.getByRole('main')).toContainText('In Progress - Branch Created');
});