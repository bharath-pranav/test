...
test('test', async ({ page }) => {
  ...
  await expect(page.getByRole('main')).toContainText('Defect');
  await expect(page.getByRole('main')).toContainText('QA Execution');
  await expect(page.getByRole('main')).toContainText('QA Test case Review');
  await expect(page.getByRole('main')).toContainText('Frontend Development');
});