...
test('test', async ({ page }) => {
  ...
  await page.getByTitle('Click to view detailed metrics for Aakash S').click();
  await expect(page.getByRole('main')).toContainText('Bug Analysis by Severity');
  await expect(page.getByRole('list')).toContainText('Bugs Created');
  await expect(page.getByRole('list')).toContainText('Bugs Fixed');
});