...
test('test', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Team Filter' }).dblclick();
  await page.getByRole('button', { name: 'Clear' }).click();
  await page.locator('.flex-1 > .flex').first().click();
  await page.locator('.fixed.inset-0').click();
  await page.getByRole('button', { name: 'Repository Filter' }).click();
  await page.locator('div').filter({ hasText: /^Asura\.ai$/ }).first().click();
  await page.locator('.fixed.inset-0').click();
