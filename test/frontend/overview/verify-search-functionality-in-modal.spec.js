...
test('test', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Team Filter' }).click();
  await page.getByRole('button', { name: 'Clear' }).click();
  await page.getByRole('textbox', { name: 'Search teams' }).click();
  await page.getByRole('textbox', { name: 'Search teams' }).fill('sparrow');
  await page.locator('div').filter({ hasText: /^sparrow-crm$/ }).first().click();
  await page.locator('div').filter({ hasText: /^sparrowdesk$/ }).first().click();
  await page.locator('div').filter({ hasText: /^sparrowgenie$/ }).first().click();
  await page.locator('div').filter({ hasText: /^Team Filter$/ }).click();
  await page.getByRole('button', { name: 'Close' }).click();
  await expect(page.getByRole('main')).toContainText('Team Filter3');
  await expect(page.getByRole('main')).toContainText('PR Cycle Time');
  await expect(page.getByRole('main')).toContainText('PR Metrics by Size');
